...
export class ModuleEffects {
  ...
  loadModules$ = this.actions$.ofType(moduleActions.ModuleActionTypes.LoadAll)
    .pipe(
      switchMap(() => {
        return this.moduleService.all().pipe(
          map(response => new moduleActions.LoadAllSuccess(response.modules)),
          catchError(e => of(new moduleActions.LoadAllFailed(e.error)))
        )
      })
    )
